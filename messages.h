#ifndef MATRICES_MESSAGES_H
#define MATRICES_MESSAGES_H

#define DEBUG

#define MSG_GET_AMOUNT "Podaj ilość macierzy do przemnożenia: "
#define MSG_GET_ROWS "Podaj ilość wierszy: "
#define MSG_GET_COLUMNS "Podaj ilość kolumn: "
#define MSG_GET_FILE_NAME "Podaj nazwę pliku, do którego zostanie zapisana macierz: "

#define MSG_MULTIPLYING_MATRICE "Mnożenie macierzy..."
#define MSG_SAVING_MULTIPLIED_MATRICE "Zapisywanie pomnożonej macierzy..."
#define MSG_SUCCESS "Pomyślnie pomnożono macierze"

#define MSG_ERR_WRONG_DATA "PODANO ZŁE DANE"
#define MSG_ERR_MEMORY_ALLOC_FAIL "NIE ZAALOKOWANO PAMIĘCI"
#define MSG_ERR_FILE_CREATION_FAIL "NIE UDAŁO SIĘ UTWORZYĆ PLIKU"
#define MSG_ERR_FILE_EXISTS "PLIK O TAKIEJ NAZWIE JUŻ ISTNIEJE"
#define MSG_ERR_FILE_DOESNT_EXIST "PLIK NIE ISTNIEJE"
#define MSG_ERR_FILE_POPULATION_FAIL "NIE UDAŁO SIĘ WYPEŁNIĆ PLIKU"
#define MSG_ERR_MATRICES_WRONG_SIZE "PODANO ZŁE ROZMIARY TABLIC"

#endif //MATRICES_MESSAGES_H
